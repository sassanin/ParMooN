...
void ExampleFile()
{
  OutPut("Example: Poiseuille_CST_Oldroyd.h" << endl) ;
}
...
//     param = parameters[i];
  ...
  u2y = 0;
    ...
    coeff[0] = nu;
    coeff[1] = (u1*tau1x) + (u2*tau1y)  + (nu*(tau1)) - (2*(tau1*u1x + tau2*u1y));
      
    coeff[2] =  (u1*tau2x) + (u2*tau2y)  + (nu*tau2)  - (tau1*u2x + tau3*u1y);
    
    coeff[3] =  (u1*tau3x) + (u2*tau3y)  + (nu*(tau3)) - (2*(tau2*u2x + tau3*u2y));
