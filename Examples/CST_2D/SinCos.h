...
void ExampleFile()
{
  ...
  OutPut("Example: SinCos_CST_Oldroyd.h ") ;
}
               ...
               double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[0] = nu;
    coeff[1] = (u1*tau1x) + (u2*tau1y) - 2*(tau1*u1x + tau2*u1y) + nu*(tau1);
    coeff[2] = (u1*tau2x) + (u2*tau2y) - (tau1*u2x + tau3*u1y) + (nu*tau2);
    coeff[3] = (u1*tau3x) + (u2*tau3y) - 2*(tau2*u2x + tau3*u2y) + nu*(tau3);
