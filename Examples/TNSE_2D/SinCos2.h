...
void InitialU2(double x, double y, double *values)
{
  ...
  values[0] = exp(-0.1*t)*-Pi*y*cos(Pi*x);
...
void InitialP(double x, double y, double *values)
{
  ...
  values[0] = exp(-0.1*t)*sin(Pi*x)*cos(Pi*y);
...
{
    double t=TDatabase::TimeDB->CURRENTTIME, t1 = exp(-0.1*t);
               ...
               double **parameters, double **coeffs)
{
  ...
  double t = TDatabase::TimeDB->CURRENTTIME, t1 = exp(-0.1*t), t2 = -0.1*exp(-0.1*t);
