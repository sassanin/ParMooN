...
//   cout<<"chek---1"<<endl;
  ...
  if (CellTree == NULL)
   ...
   for (i=0;i<N_RootCells;i++)
   {
     ...
     dat >> v1 >> v2 >> v3 >> v4 >> CellMarker;  
     Tetrahedrals[4*i    ] = v1;
     Tetrahedrals[4*i + 1] = v2;
     Tetrahedrals[4*i + 2] = v3;
     Tetrahedrals[4*i + 3] = v4;   
//      for(j=0;j<N_Domains;j++)
//      {
//      if (CellMarker==j+1)
//      {
...
//      }     
   ...
   memset(PointNeighb, 0, N_Vertices*SizeOfInt);     
     ...
     for (i=0;i<4*N_RootCells;i++)
     PointNeighb[Tetrahedrals[i]]++; //position in the array gives the vertex index and value gives the number of occurances in tetrahedrals(common vertex to a number of tetrahedrals)

   maxEpV = 0;
   for (i=0;i<N_Vertices;i++)
     if (PointNeighb[i] > maxEpV) maxEpV = PointNeighb[i]; // maximum number of edges conected to a vertex(amng all vertices)
   delete [] PointNeighb;
 
#ifdef _MPI    
     if(rank==0)   
#endif   
   cout<<"max edges per vertex "<< maxEpV<<endl;
   PointNeighb = new int[++maxEpV * N_Vertices];
   memset(PointNeighb, 0, maxEpV*N_Vertices*SizeOfInt);
//    for(i=0;i<)
   

//    cout<<"chek PointNeighb "<<maxEpV*N_Vertices<<endl;
...
delete [] PointNeighb;
 cout<< "Tetrameshcreate successful " <<"\n";
}
