...
void getboundarytag(int a, int b, int c, int NBdfaces, int *Bdfaces, int &CompID) // used in TetrameshCreate()
{
  int i,a1,b1,c1,marker;
 for(i=0;i<NBdfaces;i++)
 {
   a1 = Bdfaces[4*i];
   b1 = Bdfaces[4*i+1];
   c1 = Bdfaces[4*i+2];
   marker = Bdfaces[4*i+3];
...
{
  int i, j, k, l, dimension, N_Vertices, NBdfaces, *Bdfaces, BoundaryMarker;
  int N_Faces, N_RootCells, neib0marker, neib1marker;
  int v1, v2, v3, v4, CellMarker, RefLevel=0, *Tetrahedrals, *PointNeighb, maxEpV, *Tetrahedrals_temp;
   ...
   for(i=0;i<N_Cells;i++)
    {
      ...
      N_Faces = cell->GetN_Faces();      
      Tetrahedrals_temp = Tetrahedrals+4*i;
       for(jj=0;jj<N_Faces;jj++)
         if(cell->GetJoint(jj) == NULL)
          {
           ...
           a = Tetrahedrals_temp[TmpFV[jj*MaxLen]];
           b = Tetrahedrals_temp[TmpFV[jj*MaxLen + 1]];
           c = Tetrahedrals_temp[TmpFV[jj*MaxLen + 2]];  
           Neib[0] = -1;
           Neib[1] = -1;
           CurrNeib = 0;
             ...
             if (CurrNeib == 2)
	     {
	       ...
	       break; //break from j<=len1 loop
	       else
	       {
		getboundarytag(a,b,c, NBdfaces, Bdfaces, CompID);
		bdface = new TBdPlane(CompID);
	      bdface->GetTSofXYZ(NewVertices[a]->GetX(), NewVertices[a]->GetY(), NewVertices[a]->GetZ(), T[1], S[1]);
              bdface->GetTSofXYZ(NewVertices[b]->GetX(), NewVertices[b]->GetY(), NewVertices[b]->GetZ(), T[2], S[2]);
              bdface->GetTSofXYZ(NewVertices[c]->GetX(), NewVertices[c]->GetY(), NewVertices[c]->GetZ(), T[3], S[3]);
// 	      Joint = new TBoundFace(bdface, T, S);
	      Joint = new TInterfaceJoint3D(bdface, T, S, neib0, neib1);
// 	      TBoundComp3D *bdcomp, double *param1, double *param2, TBaseCell *neigh0, TBaseCell *neigh1
	       }
	     }
            } // for (j=1;j<=len1;j++)        
        ...
        if(CurrNeib==1)
         {  
	   getboundarytag(a,b,c, NBdfaces, Bdfaces, CompID);
// 	   cout<<"compid"<<CompID<<endl;
	   bdface = new TBdPlane(CompID);
           if(bdface->GetTSofXYZ(NewVertices[a]->GetX(), NewVertices[a]->GetY(), NewVertices[a]->GetZ(), T[1], S[1]) 
	                                            ||
              bdface->GetTSofXYZ(NewVertices[b]->GetX(), NewVertices[b]->GetY(), NewVertices[b]->GetZ(), T[2], S[2])
	                                            ||
              bdface->GetTSofXYZ(NewVertices[c]->GetX(), NewVertices[c]->GetY(), NewVertices[c]->GetZ(), T[3], S[3]) )
